...
describe("inventory pocket", () => {
  ...
  it("puts elixir 93 on the belt, leaves leftover in bag, and reconnects", async () => {
    ...
    expect(objectBlock(init["user|bag"])).toMatchObject({ amount: 2, total: 3 });
    ...
    expect(objectBlock(putOn["user|bag"])).toMatchObject({ amount: 2, total: 3 });
  ...
  it("denies pocket PUT_ON and PUT_OFF while a hunt fight is active", async () => {
    ...
    expect(objectBlock(bag["user|bag"])).toMatchObject({ status: 100, amount: 2, total: 3 });
