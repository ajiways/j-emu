...
describe("bag USE actions", () => {
  it("sets actions=7 when artifact_actions has USE and slotMask is 0", () => {
    ...
    expect(bagActionsFor(meat.slotMask, meat.useAction !== undefined, false)).toBe(
      FLAG_DROP | FLAG_SELL | FLAG_USE,
    );
    expect(bagActionsFor(meat.slotMask, meat.useAction !== undefined, false)).toBe(7);
  });
  ...
  it("does not set USE for empty artifact_actions", () => {
    ...
    expect(bagActionsFor(glove.slotMask, glove.useAction !== undefined, false)).toBe(11);
  });

  it("omits PUT_ON when the instance is broken", () => {
    expect(bagActionsFor(32, false, true)).toBe(FLAG_DROP | FLAG_SELL);
  });
});
