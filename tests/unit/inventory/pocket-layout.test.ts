...
describe("planPutOnPocket", () => {
  it("splits a bag stack that exceeds pocketCntMax into an empty slot", () => {
    const item = new InventoryItem(100_000, 1, 93, 2, { kind: "bag" });
    ...
    expect(mutation.create).toEqual([
      { heroId: 1, artifactId: 93, quantity: 1, location: { kind: "pocket", position: 1 } },
    ]);
    expect(mutation.delete).toEqual([]);
  });

  it("merges a bag orb into an incomplete pocket stack and keeps the leftover", () => {
    const pocket = new InventoryItem(100_000, 1, 99, 7, { kind: "pocket", position: 1 });
    const bag = new InventoryItem(100_001, 1, 99, 8, { kind: "bag" });
    const mutation = planPutOnPocket({
      items: [pocket, bag],
      itemId: bag.id,
      definition: orb,
  ...
  it("splits a bag orb larger than pocketCntMax into an empty slot", () => {
    const item = new InventoryItem(100_000, 1, 99, 15, { kind: "bag" });
    const mutation = planPutOnPocket({
      items: [item],
      itemId: item.id,
      definition: orb,
    ...
    expect(mutation.create).toEqual([
      { heroId: 1, artifactId: 99, quantity: 10, location: { kind: "pocket", position: 1 } },
    ]);
  });

  it("swaps different pocket stacks", () => {
    const first = new InventoryItem(100_000, 1, 93, 1, { kind: "pocket", position: 1 });
    const second = new InventoryItem(100_001, 1, 99, 10, { kind: "pocket", position: 2 });
    const mutation = planPutOnPocket({
      items: [first, second],
      itemId: first.id,
      definition: elixir,
  ...
  it("displaces a different artikul from the target cell back to the bag", () => {
    const occupant = new InventoryItem(100_000, 1, 99, 10, { kind: "pocket", position: 1 });
    const incoming = new InventoryItem(100_001, 1, 93, 1, { kind: "bag" });
    const mutation = planPutOnPocket({
  ...
  it("rejects a paperdoll glove on the belt", () => {
    const glove = new InventoryItem(100_000, 1, 9095, 1, { kind: "bag" });
    expect(() =>
      planPutOnPocket({
        items: [glove],
        itemId: glove.id,
...
describe("planMergeBagStacks", () => {
  it("merges a pocket PUT_OFF remainder into an existing bag stack", () => {
    const bag = new InventoryItem(100_000, 1, 93, 1, { kind: "bag" });
    const incoming = new InventoryItem(100_001, 1, 93, 1, { kind: "bag" });
