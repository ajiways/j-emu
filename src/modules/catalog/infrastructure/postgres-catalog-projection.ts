...
import type {
  CatalogBattlegroundMaterialization,
...
export class PostgresCatalogProjection implements CatalogProjection {
  ...
  async materialize(releaseId: string, documents: CatalogMaterialization): Promise<void> {
    ...
    await insertBotSpellBooks(session, releaseId, documents.bots);
  ...
  async materializeBattlegrounds(
    releaseId: string,
    documents: CatalogBattlegroundMaterialization,
  ): Promise<void> {
    await insertBattlegrounds(this.database.session(), releaseId, documents.battlegrounds);
  }

  async materializeProfessions(
    releaseId: string,
