...
export const listings = auctionSchema.table(
  "listings",
  {
    ...
    magicId: integer("magic_id").notNull(),
    requiredUpgradeId: integer("required_upgrade_id").notNull(),
  },
  (table) => [
    check("listings_id_check", sql`${table.id} > 0`),
    check("listings_kind_check", sql`${table.kind} IN ('lot', 'tender')`),
    check(
    ...
    check("listings_amount_check", sql`(${table.amount} > 0) OR (${table.status} <> 'open')`),
    ...
    check(
      "listings_original_item_id_check",
      sql`(${table.originalItemId} > 0) OR (${table.kind} = 'tender')`,
    ),
    ...
    check("listings_whole_stack_only_check", sql`${table.wholeStackOnly} IN (0, 1)`),
    check("listings_required_durability_check", sql`${table.requiredDurability} >= 0`),
    check("listings_required_durability_max_check", sql`${table.requiredDurabilityMax} >= 0`),
    check("listings_magic_id_check", sql`${table.magicId} >= 0`),
    check("listings_required_upgrade_id_check", sql`${table.requiredUpgradeId} >= 0`),
