...
export class AuctionLotCommand implements OaCommand {
  static readonly key = "auction|lot";
  readonly key = AuctionLotCommand.key;
  ...
  async execute(accountId: number, envelope: ObjectActionEnvelope): Promise<OaEncodedResponse> {
    try {
      ...
      const page = await this.board.lots(parseAuctionSearch(envelope.form));
      return {
        kind: "nested",
