...
export class AuctionMyTendersCommand implements OaCommand {
  static readonly key = "auction|my_tenders";
  readonly key = AuctionMyTendersCommand.key;
  ...
  async execute(accountId: number): Promise<OaEncodedResponse> {
    ...
    const rows = await this.board.myTenders(hero.id);
    return {
      kind: "nested",
