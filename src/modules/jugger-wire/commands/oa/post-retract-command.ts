...
import { postRetractMutation } from "../../application/post-claim-mutation.ts";
import type { OaCommand, OaEncodedResponse } from "./oa-command.ts";
import type { ObjectActionEnvelope } from "./object-action-envelope.ts";

export class PostRetractCommand implements OaCommand {
  static readonly key = "post|retract";
  readonly key = PostRetractCommand.key;
  ...
  async execute(accountId: number, envelope: ObjectActionEnvelope): Promise<OaEncodedResponse> {
    try {
      ...
      await this.claim.retract({ heroId: hero.id, letterId });
      const rows = await this.mail.listInbox(hero.id);
      return {
        kind: "flat",
        blocks: postRetractMutation(
