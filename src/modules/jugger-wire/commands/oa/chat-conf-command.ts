...
export class ChatConfCommand implements OaCommand {
  static readonly key = "chat|conf";
  readonly key = ChatConfCommand.key;
  ...
  async execute(accountId: number): Promise<OaEncodedResponse> {
    return flatBlockWithState(this.bootstrap, accountId, this.key, this.sheet.chatConf(accountId));
  }
}
