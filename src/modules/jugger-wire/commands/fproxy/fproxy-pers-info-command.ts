...
export class FproxyPersInfoCommand implements FproxyCommand {
  static readonly key = "persInfo";
  readonly key = FproxyPersInfoCommand.key;

  decode(frame: unknown): FightCommand {
    const record = requireFightObject(frame);
    if (record["rc"] !== "persInfo") {
    ...
    return { kind: "pers-info", sequence };
  }
}
