...
- Репутация Радвея **5** есть (REP-01, product частично).
  ...
  восемь engine-квестов, USE 584, lazy 06:00 wipe, `book|quest_delete`,
...
пускает `kind:"pvp"` same-copy; `startPvp` несёт copy id и `fightFlags`. Injected `CombatDelay`
(dueAt + cancel по duel token `${fightId}:${lo}:${hi}`) и `CombatWake` для fproxy. Loadout snapshot
...
`OPEN_STORE` — QST-ENG-05 landed: leftover op, composition штатный ComeIn, не
...
`activateCandidate`, `status`, EDT-02 `readDocument` / `listKeys`. File
`seed`/`publish` — bootstrap bundle, не editor. HTTP `/operator/content/*`
в `jugger-wire`.
