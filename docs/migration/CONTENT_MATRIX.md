...
| DATA-01 / character progression       | `jgr-emu/src/gameData.ts`, `jgr-emu/docs/CHARACTER_STATS.md`, `_research/from_register`, `_research/reg_6lvl`; normalized source manifest records evidence rank and digest                               | `catalog` owns authored curve; `character` owns player EXP/level/skills    | Planned normalized level-boundary and per-level managed-skill projection; `CORPUS`                                       | Levels start at 1; managed IDs are STR/RAG/DEX/DEF/VIT/MPMAX and must exist in skill definitions                   | Validator requires contiguous non-overlapping EXP ranges, identical complete managed set per level, integer values and positive VIT/MPMAX                    | DATA-01 skill definitions                                       | Exactly 8 boundaries and 48 managed values frozen in `CHARACTER.md`; L1–L6 are confirmed, L7–L8 carry `legacy behavior / extrapolated`; decoder manifest records normalized/source digests                                                                                                           | CHR-01 grantExperience and hero creation                       | Current v4 has 8 boundaries but no normalized per-level naked values or whole-curve validation   |
