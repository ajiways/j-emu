...
Текущая БД содержит только **минимальный playable slice v8**, а не полный
контент игры или цикла 1–8. Artifacts: **9095**, **93**, **99**, **77**. Медальон 209 и
патронташ не входят:

- 4 artifacts: `9095`, `93`, `99`, `77`;
- 1 bot: `2`;
- 1 area: `503`;
- 1 hunt spawn: `50310`;
...
| Статус        | Значение                                                                                                                      |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| `ACTIVE-MIN`  | Тип материализуется из активной playable-slice/v8 release, но данных только для минимального bootstrap/fight/pocket/USE smoke |
...
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| DATA-01 / bootstrap bundle            | `j-emu/content/playable-slice.json`, `common-conf.json`; вручную выделенный baseline с schema `playable-slice/v8` + artifact `priceMinor`/`flags`/`bagStack`/`artifact_actions` на 9095/93/99/77         | `catalog` + content publication                                            | `catalog.artifacts`, `bots`, `skill_definitions`, `level_boundaries`, `level_skill_values`, `appearance_presets`, game-wide docs; `ACTIVE-MIN` | IDs перечислены в bundle; artifact skill refs должны существовать в skills; area/hunt refs — ниже                  | v8 parser/validator/materializer требуют priceMinor/flags/bagStack/artifact_actions                                                                          | нет                                                             | Ровно 4 artifacts (9095, 93, 99, 77), 1 bot, 1 area, 1 spawn, 11 skills, 8 levels, 48 managed skill rows, 1 appearance и singleton docs; checksum текущего bundle                                                                                                                                    | bootstrap, bag/equipment, CHR-01, INV-02 DROP, INV-03 pocket, INV-04 USE, minimal hunt fight | Только минимальный playable v8 slice (9095, 93, 99, 77); 209 и патронташ не входят               |
| DATA-01 / character progression       | `jgr-emu/src/gameData.ts`, `jgr-emu/docs/CHARACTER_STATS.md`, `_research/from_register`, `_research/reg_6lvl`; normalized source manifest records evidence rank and digest                               | `catalog` owns authored curve; `character` owns player EXP/level/skills    | `catalog.level_boundaries` + `level_skill_values`; `ACTIVE-MIN`                                                                                | Levels start at 1; managed IDs are STR/RAG/DEX/DEF/VIT/MPMAX and must exist in skill definitions                   | Validator requires contiguous non-overlapping EXP ranges, identical complete managed set per level, integer values and positive VIT/MPMAX                    | DATA-01 skill definitions                                       | Exactly 8 boundaries and 48 managed values frozen in `CHARACTER.md`; L1–L6 are confirmed, L7–L8 carry `legacy behavior / extrapolated`; decoder manifest records normalized/source digests                                                                                                           | CHR-01 grantExperience and hero creation                                                     | Playable v6 publishes the frozen L1–L8 curve with whole-curve validation and progression digest  |
