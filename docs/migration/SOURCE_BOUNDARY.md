...
Текущая БД `j-emu` содержит только минимальный `playable-slice/v5`; INV-02
coding переведёт его в `v6` без импорта корпуса DATA-02. Наличие publication
pipeline не является свидетельством полного импорта этой границы.
