...
- [x] CMB-05: голый L1 vs Грызль **50310** — урон на полоске HP не
  ...
  CEF 2026-09-17.
  ```
- [x] CMB-05: Ущелье 501, три dump-hunt — Хисса **50101** (STR 15), дух
  ```
  **50102** (STR 35), рыжий грызль **50103** (STR 45); разные удары,
  не копия Грызля.
  ```
- [x] CMB-06: Хисса **50101** — плевок `magic_direct` вместо только
  ```
...
- [ ] CMB-10: NPC 271, взять «Ростер ритуала», бой flags 8 (Хисса vs
  ...
  ```
- [x] CMB-11: JOIN `{team:2}` и HELP на цель team 2 входят в тот же
  ```
  ...
  204 stale.
  ```
- [x] CMB-12: три героя на 50310 — A vs Грызль, B occupied ATTACK_BOT
  ```
...
- [x] CMB-13: два героя team 1 на 50310 — A vs Грызль, B occupied
  ...
  CEF 2026-09-17.
  ```
- [x] CMB-14: голый L1 vs Грызль 50310 — на полоске видны dodge/block/crit
  ```
  (`react` 1/6/14), не только hit.
  ```
- [x] CMB-15a: Хисса 50101 — instant kind-1 с MAGRES, не копия STR/10.
...
- [ ] CMD-?: One hit spell
  ```
  Не пропадает иконка спелла который длится ровно 1 удар, пропадает
  позже, не сразу после удара. Иконка орба не появилась пока этот спелл активен.
  ```
