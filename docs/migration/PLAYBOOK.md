...
3. **Gate-решение о статусе** — определяется правилами ниже, не запросом:
   ...
   - gates зелёные, CEF не прогнан, но production consumer уже есть →
     исключение §8 не применяется: capability остаётся открытой, CEF — это
...
- CEF обязателен для этой capability, но технически недоступен (нет
  ...
  закрывается «по-тихому» через exception §8, если production consumer уже
  есть;
