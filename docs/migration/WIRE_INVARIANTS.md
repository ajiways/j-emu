...
Фактический состав конкретной мутации фиксируется fixture-тестом; нельзя вкладывать эти блоки внутрь `common|action`. Старый клиент не гарантирует самостоятельную полную пересборку UI по одному `{status:100}`. `user|view.body` и `fight|conf.persSelf_body` — одна и та же строка Unity-оверлея (`armor(<f_body tokens>);head(...);skin()`), не набор иконок; Flash локально рисует 3D из `view.body`, а бой — из `persSelf_body`. Источники: [`docs/PROTOCOL.md`](../../../jgr-emu/docs/PROTOCOL.md), [`docs/ARCHITECTURE.md`](../../../jgr-emu/docs/ARCHITECTURE.md).
...
После fproxy `fightFinish` клиент ожидает `fight|exit` через realtime; без него result flow может ждать около 60 секунд. `fight|exit` не должен обгонять кадр `fightFinish`: loot/chat/exit ставятся на esrv только после того, как fproxy poll забрал `finished`. Если poll забрал `finished` до конца `persistFinished`, loot/exit остаются held и выпускаются сразу после persist — иначе ResultWaiting висит ~60 с. `fight|exit` идёт внутри esrv personal packet (`2:<accountId>`). Когда `fight|loot` и `fight|exit` готовы вместе, в тот же object входят synced HUD (`user|unitframe`, `user|conf`, `user|bag`, `state`), чтобы Flash обновил HP/`hp_time` и деньги без дополнительного OA. Ответ `fight|finish` не должен без подтверждённого fixture самовольно включать `common|area_conf`, поскольку это ломало ResultWaiting в просмотренном контракте. `fight|finish` отдаёт finish, `fight|info` (`fight.id` строка = numeric `fight|loot.fight_id`, плюс `share`/`macroses` SHARE), `fight|conf.expire=0`, unitframe, bag, view, magic, skills и `state`.
